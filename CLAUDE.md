...
| `pet/` | Jarvis the pet (UI-free): `model.py` (stats, XP/levels, care actions, `pet.json` persistence), `sprites.py` (fur palettes, text kitty, half-block pixel cats, pen stage compositor) |
...
- **Jarvis the pet**: a kitty that lives in a pen docked at the bottom of the sidebar (`tui/pet_pen.py:PetPen`) — it wanders, naps when idle, paces while a turn runs, chases clicks and the yarn ball; click it to pat, click the floor to call it, `pat · feed · play · nap · trick` buttons below, name row → the `/pet` card (`tui/pet_modal.py`). With the sidebar hidden it moves into the composer (`tui/pet_widget.py:PetBuddy` + a one-row `PetBubble` overlay in the always-blank row above the composer). `tui/mixins/pet.py:PetMixin` wires it to the app: turn start/finish (`_begin_turn`/`_turn_done`), tool results (`console_shim._tool_done`), typing (`mixins/file_ref.on_text_area_changed`), 2 s nudges from `_slow_refresh` (stretch break after 90 min, context ≥ 85 %, late night, hungry — rate-limited). Stats decay from wall-clock timestamps (`Pet.tick`), never below gentle floors; XP from turns/tools/edits. State: `~/.config/harness-agent/pet.json`; settings `pet.enabled`, `pet.nudges`. `/pet …` subcommands (`commands/pet.py`) run on the worker thread and animate the UI through `pet.set_reaction_hook`. **Tests** must not touch the real `pet.json` — `tests/conftest.py` redirects `PET_FILE` for every test.
