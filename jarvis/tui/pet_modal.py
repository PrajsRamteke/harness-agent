"""The ``/pet`` card — Jarvis up close: stats, level, and things to do together."""
...
def stats_text(pet, *, buddy_on: bool = True) -> Text:
    ...
    out.append(f"{age} · {sprites.FUR_LABELS.get(pet.fur, pet.fur)}", style=ui.FG_DIM)
    if not buddy_on:
        out.append(" · hidden from the input box", style=ui.FG_DIM)
    out.append("\n")
    bits = [(pet.count("turns"), "turns"), (pet.count("edits"), "edits"),
            (pet.count("pats"), "pats"), (pet.count("snacks"), "snacks")]
...
class PetCardScreen(TuiModalScreen[None]):
    ...
    def _current(self) -> tuple[str, float, int, str]:
        ...
        if get_pet().napping():
            return "sleep", t, 0, ""
        # An occasional curious glance around.
        cycle = t % 7.0
    ...
    def _tick(self) -> None:
        ...
        pet = get_pet()
        lines = sprites.big_cat(anim, t, pet.fur, look=look, snack=snack)
        key = tuple(line.plain for line in lines) + (anim, pet.fur, ui.active_theme())
        if key == self._key:
            return
        self._key = key
        try:
    ...
    def action_fur(self) -> None:
        pet = get_pet()
        pet.fur = sprites.next_fur(pet.fur)
    ...
    def action_toggle_buddy(self) -> None:
        ...
        on = not enabled()
        toggle(on)
        self._say = ("back in the input box ♥" if on
